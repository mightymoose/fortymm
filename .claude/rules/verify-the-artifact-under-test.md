...
| **A PR opened as a draft** | Every workflow here is `pull_request`-triggered (`push` is filtered to `main`), so a PR whose `opened` and `synchronize` events all landed while it was a draft can reach "ready" with **zero** workflow runs. The PR page still shows green — from Vercel alone | `gh run list --branch <branch>` must be non-empty *before* you read `gh pr checks` as a gate. Push a commit **after** marking it ready to fire `synchronize`; `ready_for_review` and `reopened` are not enough |
