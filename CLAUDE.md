...
**A human releases work from Review to Testing** — see `.claude/rules/the-review-gate.md`. `review-next-ticket` posts a decision comment on the PR and stops; `implement-ticket-end-to-end` holds a bounded watch for an `LGTM` from `mightymoose`; `test-next-ticket` refuses without one.
