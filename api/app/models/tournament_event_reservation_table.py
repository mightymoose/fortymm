...
class TournamentEventReservationTable(Base):
    ...
    reservation's ``table_ids`` JSONB array (ADR 20260801, "the tournament-scoping stops
    at the join table").
    ...
    **Three foreign keys, and this row is back where it started.** It was parented on a
    reservation, which ADR 20260815 re-parented onto a stage — dragging this row onto
    ``(stage_id, reservation_id)`` and forcing a *fourth* leg, ``(event_id, stage_id)``,
    to close the gap that indirection opened. A reservation hangs off the event
    directly, so the stage drops out of this row entirely and the fourth leg goes with
    it:
    ...
    **All three delete rules are CASCADE, and one of them is the ADR's asymmetry.**
    ...
    #: ``app.tournament_reservations`` sets ``.event = event`` on the still-unsaved
    #: object graph and lets the unit of work populate this column once the event's
    #: INSERT returns, the same mechanism that populates ``reservation_id`` through
    ...
    #: key requires. The reservation-era version of this row needed no such
    #: declaration because its parent leg led with ``stage_id`` and touched no column
    #: this one writes.
