    ...
    **The label is derived, not stored.** A group had a ``name`` for as long as it was
    ...
    def reservation(self) -> "TournamentEventReservation":
        ...
        *other* path would find it unloaded and lazy-load it — which is a
        ...
        Total on anything the **application** wrote: ``app.tournament_pools`` is the
        one write seam and every arm of it writes a group and a reservation together.
        That is an invariant of the seam, not a constraint the database enforces — the
        join table has no group-side NOT NULL, and a direct-to-database seed can make a
        bare group (one test deliberately does, to prove the fixture's composite foreign
        key refuses a cross-stage reference). Nothing projects such a row today. The
        change that lets a group exist without a reservation has to revisit this
        property, ``reservation_link``'s non-optional type, and
        :func:`app.tournament_pools.pool_read` together.
        """
