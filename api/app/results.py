...
``app.group_finishing_order``, not here: the draw layer needs the same orders to pick a
group's qualifiers and to pair a swiss round, and this module already imports
...
table per group; :class:`SingleElimResults` (ADR-0785), whose shape is the bracket's
...
pairs each one on advance"), whose shape is a single **group-less** table over the whole
...
__all__ = [
    ...
    # ``app.group_finishing_order`` so the draw layer can reach it too, but every
...
class GroupInput:
    """One group's whole input to the standings: every entry seated in it, how many
    ...
    (partial, live standings). ``fixture_count`` is the number of the group's pairings
    that can still yield a result — every pairing except any whose match was **voided**
    (which never will) — against which ``len(outcomes)`` decides whether the group is
    **complete**. Counting voided pairings would leave a group that hit one (e.g. a
...
class FieldInput:
    ...
    empty for a round-robin group, whose byed entrant is seated in every other round and
    is not credited with anything for the one they sat out.

    It is deliberately not a shared base class with :class:`GroupInput`. One is keyed by
    a group and one is not, and one scores byes, and a base would buy a name for three
...
@dataclass(frozen=True, slots=True)
class StandingRow:
    """One entry's line in a group's standings table, at its settled rank.

    ``rank`` is 1-based and every row's is distinct (the ordering is a total one — see
    :class:`RoundRobinResults`), so position 1 is the group leader.
    """
...
class GroupStandings:
    """One group's standings: its rows in finishing order, and whether every fixture in
    it has been decided."""
...
@dataclass(frozen=True, slots=True)
class EventResults:
    """A round-robin event's results: a standings table per group, whether the whole
...
class StandingsThenFinishes:
    """A round-robin-then-knockout event's results: **one block per stage** — the group
...
class SwissStandingRow(StandingRow):
    ...
    "not applicable" while reading as a real figure. Extending it does not — a group row
    ...
    (:class:`~app.group_finishing_order.SwissTally` next door stays *composed*, and the
...
class SwissStandings:
    ...
    One table and not a tuple of them, because swiss is group-less: everybody is ranked
...
def results_for(
    ...
    **no catch-all** over an enum that holds only what runs (ADR "a draw type is a
    ...
    out a single **group-less** table over the whole field. A caller narrows the union
    ...
    **two** stage inputs where its siblings take one, because a two-stage event has two
    stages to project — its group-stage fixtures and its knockout-stage ones, told apart
...
class RoundRobinResults:
    """A round-robin event's results: a standings table per group.

    Each group is ordered by :func:`~app.group_finishing_order.finishing_order` — the
    ...
    same order to pick a group's qualifiers; this strategy is the *standings* reading of
    it, and the two cannot drift because there is only one of them (ADR 20260727).
    """

    def tabulate(self, groups: Sequence[GroupInput]) -> EventResults:
        standings = tuple(_group_standings(group) for group in groups)
        # Complete = every group's every fixture decided. ``all(())`` is vacuously true,
        # so an event with no groups at all is deliberately *not* called complete.
        complete = bool(standings) and all(group.complete for group in standings)
        champion: EntryId | None = None
        if complete and len(standings) == 1 and standings[0].rows:
            champion = standings[0].rows[0].entry_id
        return EventResults(groups=standings, complete=complete, champion=champion)
...
class RrThenKoResults:
    ...
    It computes neither stage itself. The group stage *is*
    ...
    pure single-elim get — so "an rr-then-ko event's groups stand exactly as a
    ...
    Live and partial like every other shape, in either stage independently: groups
    part-played read as a live table with an empty bracket behind them, and groups
    decided with the bracket mid-flight read as a settled table plus the finishes of
    whoever has been knocked out so far.
    """

    def tabulate(
        self, groups: Sequence[GroupInput], bracket: Sequence[BracketFixture]
    ) -> StandingsThenFinishes:
        group_stage = RoundRobinResults().tabulate(groups)
...
class SwissResults:
    ...
    **Buchholz**, then game difference, games won and the entry id (ADR "swiss standings
    ...
    The chain still lives in ``app.group_finishing_order`` beside the group one, for the
    ...
    **A bye scores as a win worth zero games** (ADR "swiss standings add Buchholz, and
    ...
    that a group's does not. The win, because sitting out is a scheduling artifact the
...
def _group_standings(group: GroupInput) -> GroupStandings:
    return GroupStandings(
        group_id=group.group_id,
        rows=_standing_rows(group.entrants, group.outcomes),
        complete=len(group.outcomes) == group.fixture_count,
    )
...
def _standing_rows(
    ...
    """A group's rows, at their settled ranks, ordered by
    :func:`~app.group_finishing_order.finishing_order`.
...
def _swiss_standing_rows(
    ...
    :func:`~app.group_finishing_order.swiss_finishing_order` — the chain with
    **Buchholz** above game difference, and the figure itself carried out beside each
    row.

    Its shared columns are filled from the same :func:`_standing_columns` a group row's
...
def _standing_row(tally: EntryTally, rank: int) -> StandingRow:
    """One tally as a **group's** table row at ``rank``. A swiss row is built next door
