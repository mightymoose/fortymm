...
def upgrade() -> None:
    ...
    op.execute(
        "UPDATE tournaments SET registration_open = (status = 'published'), "
        "registration_generation = CASE WHEN status = 'published' THEN 1 ELSE 0 END"
    )
    op.create_table(
    ...
    op.create_index(
        "ix_tournament_registration_window_changes_tournament_id",
        "tournament_registration_window_changes",
        ["tournament_id"],
    )
...
def downgrade() -> None:
    op.drop_index(
        "ix_tournament_registration_window_changes_tournament_id",
        table_name="tournament_registration_window_changes",
    )
