...
async def _make_event(
    ...
    """Returns the event, its (only) group's id and its (only) stage's id.
    ...
    group's real parent is its stage, and ``TournamentEvent.stages`` is deliberately
    not
    ...
    groups = event_groups([RESERVATION_A], event=event, tournament=tournament)
    stages[0].groups = groups
    db.add(event)
    await db.flush()
    return event, groups[0].id, stages[0].id
...
async def test_a_completed_match_projects_the_games_each_side_won(
    ...
    event, group_id, stage_id = await _make_event(
...
# --- the event's group ORDER -------------------------------------------------------
#
# ``draw_config`` seeds the snake against it, ``group_order`` is what a persisted
# fixture's ``group_position`` is resolved through, and both read it off
# ``TournamentEventStageGroup.position`` (ADR 20260801) rather than off the group id.
# The ids below are server-minted uuids, whose order is *random* — which is exactly
# why the position had to become a column: a sort that fell back to the id deals a
...
async def _groups(
    ...
    Actually persisted (unlike a throwaway in-memory object): ``draw_config`` and
    ``group_order`` both read a group's order through
    ``app.tournament_draws.event_groups`` → ``app.tournament_reservations.group_read``,
    which projects each group's mapped **reservation id** — a ``server_default``
    column only Postgres assigns, at a real flush. A group built without one is a state
    no write path produces, so this helper earns it the same way the write boundary
    does rather than faking a value the domain will never actually see."""
    tournament = Tournament(
        name="Order Cup",
        address={
            "venue": "Berkeley TT Club",
...
async def test_draw_config_orders_the_groups_by_position_not_by_id(
    db_session: AsyncSession,
    default_league: League,
) -> None:
    """``DrawConfig.group_ids`` is the sequence the snake deals against, so its order is
    the draw's seeding — get it wrong and the draw still cuts, still looks like a draw,
    and seats the wrong people together.

    The groups are stored in *reverse* positional order here, so the array order and
    the positions disagree: what comes back has to be the positions' order, which is
    neither the array's nor the ids'.
    """
...
# There is deliberately no "groups stored before ``position`` existed keep their array
# order" test any more. That case was about a JSONB object with no ``position`` key;
# groups are rows with a ``NOT NULL position`` (ADR 20260801), so the state it
# described is not one the database will hold — and there is no pre-deploy data to
# describe.


async def test_group_order_ranks_every_group_id_by_the_events_order(
    db_session: AsyncSession,
    default_league: League,
) -> None:
...
async def test_fixture_state_projects_its_groups_place_in_the_event_order(
    db_session: AsyncSession,
    default_league: League,
) -> None:
    """The bridge fills ``group_position`` from the passed lookup — the fact
    ``ready_fixtures`` groups a plan by. The **tenth** group is the discriminating
    case: it is last in the director's order and, under random uuid ids, nowhere in
    ...
    assert (
        fixture_state(fixture, None, frozenset(), group_order(event)).group_position
        == 9
    )


async def test_fixture_state_projects_no_group_position_when_there_is_no_group(
    db_session: AsyncSession,
    default_league: League,
) -> None:
    ...
    assert (
        fixture_state(ungrouped, None, frozenset(), group_order(event)).group_position
        is None
    )
    assert fixture_state(grouped).group_position is None
