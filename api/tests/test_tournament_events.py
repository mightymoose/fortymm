...
# The round trips ONE pools PATCH costs, over the whole `update_event` verb: the
...
# this verb costs real round trips and leaves every one of them green. Splitting a pool
# into a group and a reservation did exactly that, twice, and both were invisible:
#
...
#
# Together they cost 7 statements on every pools PATCH. `app.tournament_pools` is the
# one write seam, so one number here covers the whole verb.
#
...
# same commit.
EXPECTED_POOL_WRITE_STATEMENTS = 15


async def test_pool_write_statement_count_does_not_drift(
    db_session: AsyncSession,
    engine: AsyncEngine,
    default_league: League,
) -> None:
    """A pools PATCH costs a fixed number of round trips, whatever the loader graph
    underneath it happens to look like.

    Two pools, both CITED by id, so this exercises the update arm of the id-keyed diff
    — the arm that re-times and re-tables an existing reservation, and therefore the one
    that has to have the whole group -> join -> reservation -> tables chain in hand. An
    add-only or remove-only payload would touch less of it.
    ...
    async with counted_statements(engine) as (session, statements):
        await update_event(
            ...
            updates=TournamentEventUpdate.model_validate(
                {
                    "pools": [
                        {**_pool("Pool A"), "id": str(group_ids[0])},
                        {**_pool("Pool B"), "id": str(group_ids[1])},
                    ]
                }
            ),
        )
    ...
    assert len(selects) == EXPECTED_POOL_WRITE_STATEMENTS, selects
