...
- **Cause:** The chart is environment-neutral, so **every** UAT-specific value
  ...
  - `tailscale.enabled` falls back to **false**, so Helm *deletes* the tailscale
    Deployment and `fortymm-uat.<tailnet>.ts.net` stops answering. (The node
    identity survives in the `tailscale-state` Secret, so a deploy with `-f`
    brings the same node back rather than registering `fortymm-uat-1`.)
  - `nginxNodePort` goes empty, so the nginx Service takes a Kubernetes-allocated
    port instead of 30084. k3d's `8084:30084@loadbalancer` map then points at
    nothing and **UAT goes dark on both `uat.fortymm.com` and `:8084`**.
