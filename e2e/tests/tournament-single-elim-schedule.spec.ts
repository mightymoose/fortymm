...
/** The event this spec seeds and cuts — a single-elimination draw, which is un-grouped
 * end to end (ADR-0786: every one of its fixtures carries `group_id IS NULL`). */
const EVENT_NAME = 'Open Singles'

/** The venue: two tables, so the two first-round matches can run at once. Neither is
 * booked by any reservation, because the event has none — they are reachable only as
 ...
 *
 * This is the window the ADR says an un-grouped fixture is placed over, so the spec
 ...
 *
 * Before that ADR the solver placed a fixture only if it belonged to a group —
 * `if fixture.group_id is None: continue` — and a bracket has no group, so no bracket
 * match was ever given a table or a call time. A director could book tables and a window
 * for a single-elim event and nothing consumed the reservation. The ADR replaced the
 * skip with a branch: an un-grouped fixture is placed over its **event's own window**, on
 ...
 *   side cannot be placed by anyone. That is the un-grouped rule doing its job rather
 *   than placing everything indiscriminately.
 *
 * ## Seed vs UI split
 *
 * Inert scaffolding over the API (`support/tournament-api.ts`): the tournament, its two
 * tables, the group-less single-elim event and its four entrants — director-entry has no
...
test.describe('Tournament — single-elim schedule', () => {
  test('a director schedules a group-less bracket onto real tables at real times', async ({
    page,
    baseURL,
  }) => {
    ...
    const { tournamentId, eventId, tables, groups } = await seedTournament(
      director,
      name,
    ...
    for (const fixture of cutRoundOne) {
      ...
      // that a failure below is about the group rule and not about a TBD side.
    ...
    // The fixtures belong to no group, so every table and every minute below came from
    // the event's own window over the tournament's whole catalogue — the event-wide
      ...
      .poll(
        ...
        },
        {
          message:
            ...
            'no group is scheduled against its event window, on any table in the tournament',
          timeout: 180_000,
          intervals: [2_000],
        },
      )
    ...
    // first-round match is won, so the un-grouped rule places what it can and leaves the
