...
# This throwaway server is development tooling, so opt into the same explicit
# local-only fallbacks as docker-compose.dev and API CI. Ask for the
# deterministic, network-free FakeGeocoder BY NAME. GEOCODER
