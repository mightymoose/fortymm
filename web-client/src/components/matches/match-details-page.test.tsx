...
describe("MatchDetailsView", () => {
  it("renders the hero scoreline from the participant sides counts", async () => {
    ...
    server.use(http.get("*/v1/matches/m-1", () => HttpResponse.json(match)));
  ...
  it("links each scored game cell on my row to its scores/edit route", async () => {
    ...
    server.use(http.get("*/v1/matches/m-4", () => HttpResponse.json(match)));
  ...
  it('renders no-opponent matches with a "No opponent" placeholder, still scorable', async () => {
    ...
    server.use(http.get("*/v1/matches/m-solo", () => HttpResponse.json(match)));
  ...
  it("renders for spectators (no current-user side) without a Score CTA", async () => {
    ...
    server.use(http.get("*/v1/matches/m-spec", () => HttpResponse.json(match)));
  ...
  it("shows the rating change card when ratings moved", async () => {
    ...
    server.use(
      http.get("*/v1/matches/m-rated", () => HttpResponse.json(match)),
    );
  ...
  // shape), finalize-callout-active.test.tsx (mutation wiring), and
  // finalize-callout.test.tsx (the callout giving way after posting).
});
