...
function adapter(
  result: Awaited<
    ReturnType<CheckoutBrowserPaymentAdapter["confirmPayment"]>
  > = {
    kind: "submitted",
  },
...
describe("tournament checkout page", () => {
  ...
  it("shows the server quote, receipt destination, card element, and server deadline", async () => {
    ...
    expect(
      within(main).getByText("Open Singles").parentElement,
    ).toHaveTextContent("$25.00");
    expect(
      within(main).getByText("Open Doubles").parentElement,
  ...
  it.each([
    ...
    "replaces loading with an actionable error when the initial %s request fails",
    async (_request, url) => {
      serve();
      server.use(
        http.get(url, () =>
  ...
  it("renders a checkout canceled before payment creation as terminal history", async () => {
    server.use(
      http.get("*/v1/tournaments/:tournamentId/checkouts/:checkoutId", () =>
        HttpResponse.json({
          ...checkout,
  ...
  it.each([
    ...
    "does not offer receipt edits after a %s payment is terminal",
    async (paymentState, copy) => {
      ...
      expect(await screen.findByText(copy)).toBeInTheDocument();
      expect(
        screen.queryByRole("textbox", { name: /receipt email/i }),
      ).not.toBeInTheDocument();
      expect(
  ...
  it("offers receipt edits while a nonterminal checking payment still accepts updates", async () => {
    ...
    expect(screen.getByRole("textbox", { name: /receipt email/i })).toHaveValue(
      "payer@example.com",
    );
    expect(
      screen.getByRole("button", { name: /save receipt email/i }),
    ).toBeEnabled();
  });
  ...
  it.each([409, 422])(
    "maps a %s receipt mutation refusal onto the field",
    async (status) => {
      serve();
      let calls = 0;
      server.use(
        http.post(
          ...
          () => {
            ...
            return HttpResponse.json(
              {
                detail:
                  status === 409 ? { code: "closed", message: "Closed." } : [],
              },
              { status },
            );
          },
  ...
  it.each(["ready", "action_required"])(
    ...
    async (authoritativeState) => {
      ...
      await user.click(await screen.findByRole("button", { name: /pay/i }));
      expect(
        await screen.findByRole("button", { name: /pay|authentication/i }),
      ).toBeEnabled();
      await user.click(
        screen.getByRole("button", { name: /pay|authentication/i }),
      );

      expect(
        await screen.findByText("Payment needs review"),
      ).toBeInTheDocument();
      expect(
        screen.getByText(/PAY-AUTHORITATIVE-TERMINAL/),
      ).toBeInTheDocument();
      expect(
        screen.queryByRole("button", { name: /pay|authentication/i }),
      ).not.toBeInTheDocument();
  ...
  it.each([
    ...
    async (terminalState, terminalCopy) => {
          ...
          () => {
            ...
            return HttpResponse.json(
              payment({
                ...
                client_secret: null,
                support_reference:
                  terminalState === "failed" ? "PAY-RECEIPT-TERMINAL" : null,
              }),
            );
          },
        ),
      ...
      expect(
        screen.getByRole("textbox", { name: /receipt email/i }),
      ).toBeInTheDocument();
  ...
  it("enters an uncertain nonpayable state before refreshing a submitted payment", async () => {
    let statusReads = 0;
    ...
    server.use(
      ...
      http.get(
        ...
        async () => {
          statusReads += 1;
          if (statusReads === 1) return HttpResponse.json(payment());
          markRefreshStarted?.();
          await refreshGate;
    ...
    const paymentAttempt = user.click(
      await screen.findByRole("button", { name: /pay/i }),
    );
    await refreshStarted;
    const checkingWhileRefreshing = screen.queryByText(
      "Payment is still being confirmed",
    );
    const payWhileRefreshing = screen.queryByRole("button", { name: /pay/i });
    releaseRefresh?.();
    await paymentAttempt;

    expect(checkingWhileRefreshing).toBeInTheDocument();
    expect(payWhileRefreshing).not.toBeInTheDocument();
    expect(
      await screen.findByText(/payment confirmation status is uncertain/i),
    ).toBeInTheDocument();
    expect(
      screen.queryByText(/could not submit that payment/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: /pay|try card again/i }),
    ).not.toBeInTheDocument();
  ...
  it("does not carry checkout A settlement into checkout B during client-side navigation", async () => {
    const checkoutFor = (checkoutId: string) => ({
      ...
      tournament_name:
        checkoutId === "checkout-a" ? "Autumn Open" : "Spring Open",
      lines: [
        {
          event_id: "open",
          event_name:
            checkoutId === "checkout-a" ? "Autumn Singles" : "Spring Singles",
          price_cents: 3750,
        },
      ],
    });
    ...
    server.use(
      http.get(
        ...
        ({ params }) =>
          HttpResponse.json(checkoutFor(String(params.checkoutId))),
      ),
      http.post(
        "*/v1/tournaments/:tournamentId/checkouts/:checkoutId/payment",
        ({ params }) =>
          HttpResponse.json(paymentFor(String(params.checkoutId))),
      ),
      http.get(
        "*/v1/tournaments/:tournamentId/checkouts/:checkoutId/payment",
        ({ params }) => {
    ...
    expect(
      screen.getByRole("heading", { name: "Spring Open" }),
    ).toBeInTheDocument();
  ...
  it("ignores redirect claims and renders only the refetched server result", async () => {
    const reads = serve({
      statuses: [payment({ payment_state: "checking", client_secret: null })],
    });
    page.render(
      adapter(),
  ...
  it("does not prepare an abandoned checkout after its delayed route load resolves", async () => {
    ...
    const checkoutARelease = new Promise<void>((resolve) => {
      releaseCheckoutA = resolve;
    });
    const checkoutAStarted = new Promise<void>((resolve) => {
      markCheckoutAStarted = resolve;
    });
    const checkoutAResponded = new Promise<void>((resolve) => {
      markCheckoutAResponded = resolve;
    });
    ...
    await userEvent
      .setup()
      .click(screen.getByRole("button", { name: "Open checkout B" }));
    expect(
      await screen.findByRole("heading", { name: "Spring Open" }),
    ).toBeInTheDocument();
  ...
  it("shows successful and mixed event outcomes without flattening them", async () => {
    ...
    expect(
      within(main).getByText("Open Singles").parentElement,
    ).toHaveTextContent("Entry confirmed");
    expect(
      within(main).getByText("Open Doubles").parentElement,
    ).toHaveTextContent("Not admitted — refund pending");
    expect(
      within(main).getByText("Open Doubles").parentElement,
    ).toHaveTextContent("$12.50");
  });
  ...
  it("counts the final partial second and removes Pay at the exact local deadline", async () => {
    ...
    renderWithRouterContext(
      ...
      {
        initialEntries: [
          "/tournaments/tournament-1770/checkouts/checkout-1770",
        ],
      },
    );
    await act(async () => {
  ...
  it("preserves a terminal payment result after the local checkout deadline", async () => {
    ...
    expect(await screen.findByText("Payment confirmed")).toBeInTheDocument();
    expect(screen.getByText("Open Singles").parentElement).toHaveTextContent(
      "Entry confirmed",
    );
    expect(screen.getByText("Open Doubles").parentElement).toHaveTextContent(
      "Not admitted — refund pending",
    );
    expect(screen.queryByText("Checkout expired")).not.toBeInTheDocument();
  });
