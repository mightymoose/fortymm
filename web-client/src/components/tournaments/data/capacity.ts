...
/** The two — mutually exclusive — things an event's capacity can say, as a sum
...
/** The two integers a capacity reading needs. Narrower than `TournamentEvent` so
 * the tests can state a case in the two numbers it is about. */
type Capacity = Pick<TournamentEvent, 'entered' | 'maxPlayers'>

/**
 * What this event has left.
 *
 * **`>=`, not `===`** — the same comparison the server's capacity guard makes
 * (`event_is_full`, `api/app/tournaments.py`). An event can hold *more* entrants
 * than `max_players`: a director who lowers the cap under a field that has
 * already formed does not evict anybody, so `entered > maxPlayers` is a real,
 * representable state. It is full — emphatically so — and the naive
...
/** The capacity in the domain's words: how many places are left, or that there
 * are none. Singular is not a nicety — "1 places left" on the last free place is
 * the copy a player is most likely to be reading. */
...
/** How full the fill bar is drawn, 0–100. Clamped at both ends: an over-full
 * event is a full bar, never one that overflows its rail, and a `maxPlayers` of
 * zero is not a division. */
export function capacityFillPercent(event: Capacity): number {
