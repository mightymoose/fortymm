...
export interface RoundListProps {
  rounds: DrawRound[]
  /** What these rounds belong to — a group's position-derived label ("Group A"), or the
   * ungrouped block's heading. It is not rendered: it *names* each round's list for a
   * screen reader ("Round 1 fixtures in Group A"), which is the only thing that tells
   * one group's round 1 from the next group's when a rotor lists them side by side. */
  groupName: string
}
 ...
 * round-robin group has rounds holding **fewer fixtures** than the others, because the
 ...
 *
 * Shared by `GroupDraw` (a group's rounds) and `DrawPanel` (the ungrouped fixtures of a
