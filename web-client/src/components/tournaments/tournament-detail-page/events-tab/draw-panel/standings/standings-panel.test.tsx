...
describe('StandingsPanel', () => {
  ...
  it('shows a standings table per group, entrants joined to their names', () => {
    ...
    expect(page.getGroupTableNames()).toEqual(['Standings for Group A'])
  ...
  it('does NOT show a champion while the event is still being played', () => {
    ...
    // callout is absent while the group table is present.
    page.render({
      event: buildStandingsEvent({
        results: buildEventResults({
          complete: false,
          champion: null,
          groups: [buildGroupStandings({ complete: false })],
        }),
      }),
    })
  ...
  it('shows every group but NO champion for a complete multi-group event', () => {
    // A multi-group round-robin has no single champion without a knockout stage to join its
    // group winners (a later slice), so `champion` is `null` even when complete — the group
    ...
    page.render({
      event: buildStandingsEvent({
        ...
        results: buildEventResults({
          ...
          champion: null,
          groups: [
            buildGroupStandings({ groupId: groupIdFor('res-a') }),
            buildGroupStandings({
    ...
    expect(page.getGroupTableNames()).toEqual([
